...
export default function AiClient({ displayName, role }: { displayName: string; role: string }) {
  ...
  async function ask(text: string) {
    ...
    try {
      ...
      const payload = await response.json() as { text?: string; error?: string; redacted?: boolean };
      if (!response.ok || !payload.text) throw new Error(payload.error || "پاسخی دریافت نشد");
      if (payload.redacted) setPrivacyNote("برای حفظ محرمانگی، کد ملی یا شماره تماس موجود در پیام قبل از ارسال به هوش مصنوعی حذف شد.");
    ...
    <main className="ram-ai-shell" dir="rtl">
      ...
      <div className="ram-ai-frame">
        <aside className="ram-ai-side">
          ...
          <div className="ram-ai-privacy">کد ملی و شماره تماس، حتی اگر ناخواسته در پیام وارد شوند، پیش از ارسال به هوش مصنوعی در سرور حذف می‌شوند.</div>
        ...
        <section className="ram-ai-chat">
          ...
          {privacyNote && <div className="ram-ai-notice">{privacyNote}</div>}
